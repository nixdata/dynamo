...
int dmo_server_startup(struct dmo_server server) 
{
    ...
    nio_server = netcode_server_create(server.address, server.protocol, private_key, dmo_sys_time_to_double(time));
    if(!nio_server) {
        printf("ERROR: Failed to create netcode.io server.\n");
        return DYNAMO_ERROR;
    }
    netcode_server_start(nio_server, NETCODE_MAX_CLIENTS);
...
{
    netcode_server_update(nio_server, dmo_sys_time_to_double(time));

    if(netcode_server_client_connected(nio_server, 0)) {
        //netcode_server_send_packet(nio_server, 0, packet_data, NETCODE_MAX_PACKET_SIZE);
    }
    ...
    for(int client_index = 0; client_index < NETCODE_MAX_CLIENTS; ++client_index) {
        while(1) {
            i32 packet_bytes; 
            u64 packet_sequence;
            u8 *packet = netcode_server_receive_packet(nio_server, client_index, &packet_bytes, &packet_sequence);
            if(!packet) {
                break;
            }
            ...
            netcode_server_free_packet(nio_server, packet);
        }
    }
    return DYNAMO_OK;
}
